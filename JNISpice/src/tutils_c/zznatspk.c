/* zznatspk.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int zznatspk_(char *file, ftnlen file_len)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int cleard_(integer *, doublereal *), dafcls_(
	    integer *);
    doublereal omegag, speeda, mu, speedb, rgamma, ralpha, deltao, speedg;
    extern doublereal clight_(void);
    extern /* Subroutine */ int kilfil_(char *, ftnlen);
    integer myhand;
    doublereal statea[6], stateb[6], stateg[6];
    extern /* Subroutine */ int chkout_(char *, ftnlen), spcopn_(char *, char 
...
/*     CALL ZZNATSPK ( 'nat.bsp' ) */
/*     CALL FURNSH   ( 'nat.bsp' ) */

/*     CALL ZZNATPCK ( 'nat.tpc' ) */
/*     CALL FURNSH   ( 'nat.tcp' ) */
...
/* -    Test Utilities 1.0.0, 28-AUG-2008 (EDW) */

/*        Reimplementation of NATSPK removing all code */
...
/* Computing 3rd power */
    d__2 = rgamma;
    mu = d__1 * d__1 * (d__2 * (d__2 * d__2));
    speedg = rgamma * omegag;
    d__1 = -rgamma;
    vscl_(&d__1, yvec, stateg);
    vscl_(&speedg, zvec, &stateg[3]);
    s_copy(segid, "Alpha's moon Gamma", (ftnlen)40, (ftnlen)18);
    spkw05_(&myhand, &c__1001, &c__1000, "J2000", &first, &last, segid, &mu, &
	    c__1, stateg, &c_b14, (ftnlen)5, (ftnlen)40);
