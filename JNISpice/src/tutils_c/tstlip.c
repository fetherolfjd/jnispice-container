...
/* -    Version 2.0.0, 29-MAY-2010 (NJB) */

/*        Increase line size to 2000. */
...
/*     Local Variables. */
    ...
    s_copy(fail, "LEFT 1 RIGHT 78 HARDSPACE $ FLAG CASE$FAILED!", (ftnlen)120,
	     (ftnlen)45);
    if (logifp || verbos_()) {
	s_copy(messge, " ", (ftnlen)2000, (ftnlen)1);
...
/*           Set the logging style to PASS and print the case name. */
	    ...
	    t_cname__(messge, (ftnlen)2000);
	    tstlog_(messge, &c_false, (ftnlen)2000);
	} else if (number > 0) {
