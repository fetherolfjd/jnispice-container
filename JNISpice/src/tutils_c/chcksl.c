...
/*     Set placeholder message. */
    ...
    s_copy(messge, "This is a placeholder message that should have never bee"
	    "n displayed. It indicates a CHCKSL bug.", (ftnlen)320, (ftnlen)95)
	    ;
...
/*     Just make sure that the values of VAL and EXP are the same. */
	...
	*ok = *val;
	if (*ok) {
	    if (verbos_()) {
		s_copy(messge, "The value of # was TRUE as expected. ", (
			ftnlen)320, (ftnlen)37);
	    }
	} else {
	...
	*ok = ! (*val);
	if (*ok) {
	    if (verbos_()) {
		s_copy(messge, "The value of # was FALSE as expected. ", (
			ftnlen)320, (ftnlen)38);
	    }
	} else {
