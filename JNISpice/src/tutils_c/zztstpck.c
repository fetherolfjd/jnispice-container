/* zztstpck.f -- translated by f2c (version 19980913).
...
/* $Procedure ZZTSTPCK (Create a test PCK) */
/* Subroutine */ int zztstpck_(char *namepc, ftnlen namepc_len)
{
...
/*     Create a test PCK text kernel named NAMEPC. */
...
/*     NAMEPC      is the name of a PC-kernel to create. */
/*                 If a PC-kernel of the same name already exists */
/*                 it is deleted. */
...
/*      This routine creates single PC-kernel. See Particulars */
/*      for more details. */
...
/* -    Test Utilities 1.0.0, 05-DEC-2005 (EDW) */

/*        Reimplementation of TSTPCK removing all code */
/*        except that needed to write the test PCK. */

/* -& */
/* $ Index_Entries */

/*     Create test PCK files. */
...
/*     Local Variables. */
    ...
    s_copy(pck + 640, "go out of date.  It will not be updated because it is "
	    , (ftnlen)80, (ftnlen)54);
    s_copy(pck + 720, "generated automatically at run time by the routine ZZ"
	    "TSTPCK. ", (ftnlen)80, (ftnlen)61);
...
/*     Create the PCK kernel. */
	...
	r__ = rtrim_(pck + ((i__1 = i__ - 1) < 466 && 0 <= i__1 ? i__1 : 
		s_rnge("pck", i__1, "zztstpck_", (ftnlen)982)) * 80, (ftnlen)
		80);
	writln_(pck + ((i__1 = i__ - 1) < 466 && 0 <= i__1 ? i__1 : s_rnge(
		"pck", i__1, "zztstpck_", (ftnlen)983)) * 80, &unit, r__);
    }
    cl__1.cerr = 0;
    cl__1.cunit = unit;
    cl__1.csta = 0;
