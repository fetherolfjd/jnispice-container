/* chckoc.f -- translated by f2c (version 19980913).
...
/* $Procedure      CHCKOC ( Check order of an d.p. array ) */
/* Subroutine */ int chckoc_(char *name__, char *array, char *order, integer *
	...
	order_len)
{
    ...
    logical l_lt(char *, char *, ftnlen, ftnlen), l_le(char *, char *, ftnlen,
	     ftnlen), l_gt(char *, char *, ftnlen, ftnlen), l_ge(char *, char 
	    *, ftnlen, ftnlen);
...
/*     ARRAY       is the actual string array to be examined */
...
/* -    Testing Utilities 1.0.1, 05-AUG-2013 (BVS) */
...
/*     Check the order of elements of a character array. */
...
/*     So far the test case has not failed. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    fail = fail || ! (s_cmp(array + (i__ - 1) * array_len, array + 
		    i__ * array_len, array_len, array_len) == 0);
	    if (fail) {
		j = min(j,i__);
	    }
	}
	...
	i__1 = *size - 1;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    fail = fail || ! l_lt(array + (i__ - 1) * array_len, array + i__ *
	...
	i__1 = *size - 1;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    fail = fail || ! l_le(array + (i__ - 1) * array_len, array + i__ *
		     array_len, array_len, array_len);
	}
	if (fail) {
	    j = min(j,i__);
	...
	i__1 = *size - 1;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    fail = fail || ! l_gt(array + (i__ - 1) * array_len, array + i__ *
		     array_len, array_len, array_len);
	}
	if (fail) {
	    j = min(j,i__);
	...
	i__1 = *size - 1;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    fail = fail || ! l_ge(array + (i__ - 1) * array_len, array + i__ *
		     array_len, array_len, array_len);
	}
	if (fail) {
	    j = min(j,i__);
	...
	s_copy(messge, "The input array # is does not satisfy the condition"
		...
		"(@) = \"%\" /cr #($) = \"&\". ", (ftnlen)800, (ftnlen)135);
	repmc_(messge, "#", name__, messge, (ftnlen)800, (ftnlen)1, r1, (
		ftnlen)800);
	repmc_(messge, "#", name__, messge, (ftnlen)800, (ftnlen)1, r1, (
		ftnlen)800);
	repmc_(messge, "#", name__, messge, (ftnlen)800, (ftnlen)1, r1, (
	...
	repmc_(messge, "#", name__, messge, (ftnlen)800, (ftnlen)1, r1, (
		ftnlen)800);
	repmc_(messge, "?", order, messge, (ftnlen)800, (ftnlen)1, r2, (
		ftnlen)800);
	repmi_(messge, "@", &j, messge, (ftnlen)800, (ftnlen)1, (ftnlen)800);
	repmi_(messge, "@", &j, messge, (ftnlen)800, (ftnlen)1, (ftnlen)800);
	i__1 = j + 1;
	repmi_(messge, "$", &i__1, messge, (ftnlen)800, (ftnlen)1, (ftnlen)
		800);
	i__1 = j + 1;
	repmi_(messge, "$", &i__1, messge, (ftnlen)800, (ftnlen)1, (ftnlen)
		800);
	repmc_(messge, "%", array + (j - 1) * array_len, messge, (ftnlen)800, 
		(ftnlen)1, array_len, (ftnlen)800);
	repmc_(messge, "&", array + j * array_len, messge, (ftnlen)800, (
		ftnlen)1, array_len, (ftnlen)800);
	tstlog_(messge, &fail, (ftnlen)800);
    } else if (verbos_()) {
	s_copy(messge, "The relationship #(I) ? #(I+1) is true for all eleme"
		"nts of the input array # ", (ftnlen)800, (ftnlen)77);
	repmc_(messge, "#", name__, messge, (ftnlen)800, (ftnlen)1, r1, (
		ftnlen)800);
	repmc_(messge, "#", name__, messge, (ftnlen)800, (ftnlen)1, r1, (
		ftnlen)800);
