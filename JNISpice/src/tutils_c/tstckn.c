...
/*     Create the SCLK kernel. */
	...
	r__ = rtrim_(sclkbf + ((i__1 = i__ - 1) < 98 && 0 <= i__1 ? i__1 : 
		s_rnge("sclkbf", i__1, "tstckn_", (ftnlen)456)) * 80, (ftnlen)
		80);
	io___5.ciunit = unit;
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, sclkbf + ((i__1 = i__ - 1) < 98 && 0 <= i__1 ? 
		i__1 : s_rnge("sclkbf", i__1, "tstckn_", (ftnlen)457)) * 80, 
		r__);
	e_wsle();
    }
    cl__1.cerr = 0;
...
/*     The zero point of the clock is 1 Jan 1980 TDB. */
	...
	tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
		"tstckn_", (ftnlen)477)] = (et - zeropt) * 1e4;
	++j;
	tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
		"tstckn_", (ftnlen)480)] = tick[(i__2 = j - 2) < 205 && 0 <= 
		i__2 ? i__2 : s_rnge("tick", i__2, "tstckn_", (ftnlen)480)] - 
		1.;
	et += -1e7;
    }
    ++j;
    tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
	    "tstckn_", (ftnlen)487)] = 0.;
    et = 1e7;
    while(et < maxet) {
	++j;
	tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
		"tstckn_", (ftnlen)495)] = (et - zeropt) * 1e4;
	++j;
	tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
		"tstckn_", (ftnlen)498)] = tick[(i__2 = j - 2) < 205 && 0 <= 
		i__2 ? i__2 : s_rnge("tick", i__2, "tstckn_", (ftnlen)498)] - 
		1.;
	et += 1e7;
    }
    ++j;
    tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
	    "tstckn_", (ftnlen)505)] = (maxet - zeropt) * 1e4;
    nticks = j;
...
/*     Sort the ticks. */
	...
	tk = tick[(i__2 = i__ - 1) < 205 && 0 <= i__2 ? i__2 : s_rnge("tick", 
		i__2, "tstckn_", (ftnlen)523)];
	et = zeropt + tk / 1e4;
	tstatd_(&et, rot, &angvel[(i__2 = i__ * 3 - 3) < 615 && 0 <= i__2 ? 
		i__2 : s_rnge("angvel", i__2, "tstckn_", (ftnlen)525)]);
	m2q_(rot, &quatrn[(i__2 = (i__ << 2) - 4) < 820 && 0 <= i__2 ? i__2 : 
		s_rnge("quatrn", i__2, "tstckn_", (ftnlen)526)]);
    }
    nints = 1;
    starts[0] = 0.;
...
/*     10 million seconds later than those for body -9999 */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	tk = tick[(i__2 = i__ - 1) < 205 && 0 <= i__2 ? i__2 : s_rnge("tick", 
		i__2, "tstckn_", (ftnlen)552)];
	et = zeropt + tk / 1e4;
	tstatd_(&et, rot, &angvel[(i__2 = i__ * 3 - 3) < 615 && 0 <= i__2 ? 
		i__2 : s_rnge("angvel", i__2, "tstckn_", (ftnlen)554)]);
	m2q_(rot, &quatrn[(i__2 = (i__ << 2) - 4) < 820 && 0 <= i__2 ? i__2 : 
		s_rnge("quatrn", i__2, "tstckn_", (ftnlen)555)]);
    }
    nints = 1;
    starts[0] = 0.;
    ckw03_(handle, &begtim, &endtim, &inst, ref, &avflg, segid, &nticks, tick,
...
/*     but 100 million seconds later than those for object -9999 */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	tk = tick[(i__2 = i__ - 1) < 205 && 0 <= i__2 ? i__2 : s_rnge("tick", 
		i__2, "tstckn_", (ftnlen)581)];
	et = zeropt + tk / 1e4;
	tstatd_(&et, rot, &angvel[(i__2 = i__ * 3 - 3) < 615 && 0 <= i__2 ? 
		i__2 : s_rnge("angvel", i__2, "tstckn_", (ftnlen)583)]);
	m2q_(rot, &quatrn[(i__2 = (i__ << 2) - 4) < 820 && 0 <= i__2 ? i__2 : 
		s_rnge("quatrn", i__2, "tstckn_", (ftnlen)584)]);
    }
    nints = 1;
    starts[0] = 0.;
    ckw03_(handle, &begtim, &endtim, &inst, ref, &avflg, segid, &nticks, tick,
