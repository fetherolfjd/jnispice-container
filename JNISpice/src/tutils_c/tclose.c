...
/* Subroutine */ int tclose_(void)
{
    ...
    char time2[80];
    extern /* Subroutine */ int tbegf_(void);
    integer nfail, ncase;
    ...
    extern /* Subroutine */ int errdev_(char *, char *, ftnlen, ftnlen), 
	    ...
	    , ftnlen, ftnlen), cstart_(void), curtim_(char *, ftnlen), 
	    suffix_(char *, integer *, char *, ftnlen, ftnlen), tstglf_(char *
	    , ftnlen), tstfil_(char *, char *, char *, ftnlen, ftnlen, ftnlen)
	    , tstioa_(char *, ftnlen), tstget_(char *, char *, char *, ftnlen,
	     ftnlen, ftnlen), tstioh_(char *, ftnlen), tstlog_(char *, 
	    logical *, ftnlen), tstioc_(char *, ftnlen), tstlip_(void), 
	    tstlgs_(char *, char *, ftnlen, ftnlen), tstslf_(char *, ftnlen), 
	    tstwln_(char *, ftnlen), tstrul_(void);
    char bad[120];
...
/* -    Testing Utilities Version 3.1.0, 04-SEP-2013 (BVS) */

/*        Added getting and printing the current time at the very end. */
...
/*        belong in it. */
	...
	tstlog_(messge, &c_false, (ftnlen)256);
	tstwln_(" ", (ftnlen)1);
	tstlog_(time2, &c_false, (ftnlen)80);
	tstwln_(" ", (ftnlen)1);
