...
/*     T_CASE  ( NAME )  --- gives a name (possibly blank) to the */
/*                           current test case. Set the current case */
/*                           success to .TRUE. The name may be up to */
/*                           MAXCNM characters. */
...
/* -    Testing Utilities 3.0.0, 29-MAY-2010 (NJB) */

/*        Updated parameters to increase max length of test family names */
/*        from 12 characters to MAXFNM and increase max length of test */
/*        case names from 80 to MAXCNM. */
...
/*        Store the name and current case number. */
    ...
    succes = TRUE_;
    s_copy(rname, name__, (ftnlen)80, name_len);
    caseno = 0;
    nfail = 0;
...
/*        DIGIT(D(1),1)//DIGIT(D(2),2)// ... //DIGIT(D(MAXPLC),MAXPLC). */
	...
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 10) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)331)] = ' ';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 9) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)332)] = '1';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 8) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)333)] = '2';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 7) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)334)] = '3';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 6) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)335)] = '4';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 5) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)336)] = '5';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 4) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)337)] = '6';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 3) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)338)] = '7';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 2) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)339)] = '8';
	*(unsigned char *)&digit[(i__1 = i__ * 10 - 1) < 70 && 0 <= i__1 ? 
		i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)340)] = '9';
	d__[(i__1 = i__ - 1) < 7 && 0 <= i__1 ? i__1 : s_rnge("d", i__1, 
		"t_stat__", (ftnlen)341)] = 1;
    }
...
L_t_name:
    s_copy(name__, rname, name_len, (ftnlen)80);
    return 0;
...
/*        increment the case number. */
    ...
    s_copy(descr, name__, (ftnlen)1000, name_len);
    caseok = TRUE_;
    ++caseno;
...
/*              Set the first digit to zero "0". */
	    ...
	    d__[(i__1 = i__ - 1) < 7 && 0 <= i__1 ? i__1 : s_rnge("d", i__1, 
		    "t_stat__", (ftnlen)400)] = d__[(i__2 = i__ - 1) < 7 && 0 
		    <= i__2 ? i__2 : s_rnge("d", i__2, "t_stat__", (ftnlen)
		    400)] + 1;
	    *(unsigned char *)&digit[(i__1 = i__ * 10 - 10) < 70 && 0 <= i__1 
		    ? i__1 : s_rnge("digit", i__1, "t_stat__", (ftnlen)401)] =
		     '0';
...
/*              That is, make sure INCR stays .TRUE. */
	    ...
	    if (d__[(i__1 = i__ - 1) < 7 && 0 <= i__1 ? i__1 : s_rnge("d", 
		    i__1, "t_stat__", (ftnlen)407)] > 10) {
		d__[(i__1 = i__ - 1) < 7 && 0 <= i__1 ? i__1 : s_rnge("d", 
			i__1, "t_stat__", (ftnlen)409)] = 1;
	    } else {
...
L_t_cname:
    s_copy(myname, "Test Case ", (ftnlen)1000, (ftnlen)10);
    j = 10;
...
/*        Follow that up with the case description. */
    ...
    j += 2;
    s_copy(myname + (j - 1), descr, 1000 - (j - 1), (ftnlen)1000);
    s_copy(name__, myname, name_len, (ftnlen)1000);
    return 0;
