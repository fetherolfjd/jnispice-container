...
/* Subroutine */ int topen_0_(int n__, char *name__, ftnlen name_len)
{
    ...
    static char tnames[80*1000];
    extern /* Subroutine */ int qcktrc_(char *, ftnlen), errdev_(char *, char 
	    *, ftnlen, ftnlen), tstcbl_(void), repmct_(char *, char *, 
	    integer *, char *, char *, ftnlen, ftnlen, ftnlen, ftnlen), 
	    cstart_(void), suffix_(char *, integer *, char *, ftnlen, ftnlen),
...
/* -    Testing Utilities 5.0.0, 30-MAY-2010 (NJB) */

/*        Increased length of test family names from 32 to */
/*        MAXFNM. */
...
/*     Local Variables and Parameters */
	...
	for (i__ = 1; i__ <= 1000; ++i__) {
	    s_copy(tnames + ((i__1 = i__ - 1) < 1000 && 0 <= i__1 ? i__1 : 
		    s_rnge("tnames", i__1, "topen_", (ftnlen)214)) * 80, 
		    " ", (ftnlen)80, (ftnlen)1);
	}
    } else {
...
/*     summary purposes. */
	...
	s_copy(tnames + ((i__1 = ntest - 1) < 1000 && 0 <= i__1 ? i__1 : 
		s_rnge("tnames", i__1, "topen_", (ftnlen)239)) * 80, name__, (
		ftnlen)80, name_len);
    }
...
/*     Set the "old" trace value. */
    ...
    t_name__(oldnam, (ftnlen)80);
    if (s_cmp(oldnam, " ", (ftnlen)80, (ftnlen)1) != 0) {
...
/*        We need to close out the old case */
	...
	if (ok) {
	    s_copy(messge, "Passed  --- Test Family: ", (ftnlen)160, (ftnlen)
		    25);
	    suffix_(oldnam, &c__1, messge, (ftnlen)80, (ftnlen)160);
...
L_tfname:
	...
	s_copy(name__, tnames + ((i__1 = tfamc - 1) < 1000 && 0 <= i__1 ? 
		i__1 : s_rnge("tnames", i__1, "topen_", (ftnlen)365)) * 80, 
		name_len, (ftnlen)80);
	++tfamc;
    } else {
