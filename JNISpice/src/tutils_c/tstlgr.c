...
/* -     Test Utilities Version 2.0.0, 29-MAY-2010 (NJB) */

/*         Updated parameters to increase max length of test family */
/*         names from 12 characters to MAXFNM and increase max length of */
/*         test case names from 80 to MAXCNM. */

/*         Added bogus header sections to satisfy checkit. */

/* -     Test Utilities Version 1.3.0, 30-DEC-2002 (EDW) */

/*         Implemented the ZZTSTRE routine to reset the OPNERR */
/*         flag to TRUE. */
...
/*        message for this test case. */
	...
	t_name__(name__, (ftnlen)80);
	t_cname__(cname, (ftnlen)1000);
	t_fcount__(&pnum);
	t_cfail__();
	t_fcount__(&fnum);
	if (opnerr) {
...
/* Writing concatenation */
	    ...
	    i__1[1] = 80, a__1[1] = name__;
	    s_cat(ch__1, a__1, i__1, &c__2, (ftnlen)89);
	    tstwln_(ch__1, (ftnlen)89);
	    nicepr_1__(cname, "LEFT 1 RIGHT 78 HARDSPACE ^ FLAG CASE^^^:", (
		    S_fp)tstwln_, (ftnlen)1000, (ftnlen)41);
	    tstwln_(" ", (ftnlen)1);
...
/*        Reactivate the screen and log file. */
	...
	if (s_cmp(messge, " ", messge_len, (ftnlen)1) == 0) {
	    ...
	    if (show) {
		nicepr_1__(extra, hide, (S_fp)tstwln_, (ftnlen)1000, (ftnlen)
			128);
		show = FALSE_;
		tstwln_(" ", (ftnlen)1);
	    }
...
L_tstmsg:
    *(unsigned char *)&mymark[0] = *(unsigned char *)marker;
    s_copy(extra, messge, (ftnlen)1000, messge_len);
    show = TRUE_;
    return 0;
...
L_tstmsi:
    repmi_(extra, mymark, int__, extra, (ftnlen)1000, (ftnlen)1, (ftnlen)1000)
	    ;
    return 0;
...
L_tstmsd:
    repmd_(extra, mymark, dp, &c__14, extra, (ftnlen)1000, (ftnlen)1, (ftnlen)
	    1000);
    return 0;

L_tstmsf:
    repmf_(extra, mymark, dp, &c__14, "F", extra, (ftnlen)1000, (ftnlen)1, (
	    ftnlen)1, (ftnlen)1000);
    return 0;
...
L_tstmsc:
    ...
    repmc_(extra, mymark, messge, extra, (ftnlen)1000, (ftnlen)1, r__, (
	    ftnlen)1000);
    return 0;
...
/* -& */
    if (*(unsigned char *)marker != 'C' && *(unsigned char *)marker != 'c' && 
	    *(unsigned char *)marker != 'l' && *(unsigned char *)marker != 
	    ...
	    marker != 'U') {
	repmot_(extra, mymark, int__, "l", extra, (ftnlen)1000, (ftnlen)1, (
		ftnlen)1, (ftnlen)1000);
    } else {
	repmot_(extra, mymark, int__, marker, extra, (ftnlen)1000, (ftnlen)1, 
		(ftnlen)1, (ftnlen)1000);
    }
    return 0;
...
L_tstmst:
    if (*(unsigned char *)marker != 'C' && *(unsigned char *)marker != 'c' && 
	    *(unsigned char *)marker != 'l' && *(unsigned char *)marker != 
	    'u' && *(unsigned char *)marker != 'L' && *(unsigned char *)
	    marker != 'U') {
	repmct_(extra, mymark, int__, "l", extra, (ftnlen)1000, (ftnlen)1, (
		ftnlen)1, (ftnlen)1000);
    } else {
	repmct_(extra, mymark, int__, marker, extra, (ftnlen)1000, (ftnlen)1, 
		(ftnlen)1, (ftnlen)1000);
    }
    return 0;
...
/*   Reset OPNERR to TRUE. FINISH calls this routine immediately prior to */
/*   return. This action is required for those situations where the test */
/*   system runs in a persistent environment (e.g. IDL, MATLAB). */
...
/*     None of the testutil routines reset the OPNERR flag after a */
/*     a test run - indicated by a call to tclose. The original testutil */
/*     design expected test program use in the conventioanl single run */
/*     environment such as a command line shell so this failure to reset */
/*     caused no effect. The failure to reset the flag can cause system */
/*     errors when repeatedly invoking a test program using the testutil */
/*     library in a persistent environment such as IDL or MATLAB. */
...
/* -  Test Utilities Version 1.0.0, 18-JAN-2007 (EDW) */
